    ...
    <div class="container mx-auto mt-10 w-1/2 bg-white shadow-md rounded-md px-4 pt-6 pb-8 mb-4 py-8">
        <form method="POST" action="{{ route('posts.update', ['id' => $post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group flex flex-row">
                ...
                <input type="text" name="title" id="title" class="form-control w-full" required value="{{ $post->title }}">
                @error('title')
            ...
            <div class="form-group flex flex-row mt-3">
                ...
                <textarea name="body" id="body" class="form-control w-full" rows="6" required>{{ $post->body }}</textarea>
                @error('content')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="form-group">
                ...
                @if ($post->image)
                    <img src="{{ asset($post->image) }}" alt="{{ $post->title }}" style="max-width: 300px;">
                @endif
                @error('image')
